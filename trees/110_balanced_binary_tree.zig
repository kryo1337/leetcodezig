...
fn checkHeight(node: ?*const TreeNode) i32 {
...
fn isBalanced(root: ?*const TreeNode) bool {
    return checkHeight(root) != -1;
}

pub fn main() !void {}

test "balanced - balanced tree" {
...
test "balanced - unbalanced tree" {
    const allocator = std.testing.allocator;
    const root = try TreeNode.fromArray(allocator, &[_]?i32{ 1, 2, 2, 3, 3, null, null, 4, 4 });
...
test "balanced - deep unbalanced tree" {
    const allocator = std.testing.allocator;
